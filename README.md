...
3. **The Divergence** (legacy internal reading-type identifier: `synthesis`, kept for cache-key stability) — Where the Chart Is Least Negotiable, Where They Part, The Central Tension, Living the Divergence. Its context block is a single deterministic SHARED DIVERGENCE PLAN (`buildDivergencePlan`) used unchanged by all four sections: every body in both frames with degree, dignity, retrogradation, sidereal nakshatra and sign-ruler condition; concordances in two grades (full, and same-sign with a house shift); divergences ranked by interpretive weight of the body and structural size of the shift; the aspects that link them; dasha timing and sidereal yogas; and explicit reliability limits when birth time is unknown. The reading names where the systems agree and where they pull apart, names the divergences it did not walk as still unresolved, and never averages the two charts into one answer.
