...
The LLM **quality gate** (`lib/reading-quality-gate.ts`) is **on the request path.** After the first pass streams to the client, `/api/reading` calls `evaluateSection` synchronously; if the section fails and there is wall-clock budget left (`REPAIR_SKIP_THRESHOLD_MS`), a single `repairSection` pass is streamed behind an `[AXIS_REPAIRED]` marker and the client swaps the visible draft for the corrected copy. This runs under the 300 s Vercel Pro `maxDuration` — the passes had been taken off the path only while the ceiling was 60 s, which gen + eval alone breached on heavy sections. Caching is guarded so only text that is non-empty, non-truncated (`isTruncated`), gate-passing (or successfully repaired), and free of banned phrasings (`detectBannedPhrasings`) is written; truncated text never reaches the cache or ships as final.
