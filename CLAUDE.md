...
- **`lib/interpretation-engine.ts`**: derives astrological facts from chart data; formats structured context injected into every reading request. Data-driven via `PLANET_CORE`, `SIGN_DATA`, `HOUSE_DATA`, `DIGNITIES`, `NAKSHATRA_DATA` constants.
- **`lib/prompts.ts`**: system prompts at **v9.6**. `SHARED_RULES` is prompt-cached (ephemeral). `SECTION_INSTRUCTIONS` map keyed by section→planetSection. Load-bearing constraints: cusp rule (±3° of sign boundary), SELF-COMPLETE SECTIONS rule (each `/api/reading` call is stateless — no conversation history, so sections must be self-complete and must NOT reference other sections' prose), SITUATIONAL MANIFESTATION rule (anchor every named pattern to the condition that activates it — *when* it shows up, not only *where* the struggle lives), depth targets (550–750 words Sun/Moon/ASC, 300–400 secondaries). **Always bump `READING_PROMPT_VERSION` after structural changes.**
- **`lib/reading-cache.ts`**: Upstash Redis KV cache, 30-day TTL. `READING_PROMPT_VERSION = 'v9.6'` — bump to invalidate all prior cached readings.
