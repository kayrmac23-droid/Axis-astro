// lib/prompts.ts
// AXIS Production System Prompts v10.0
// Architecture:
...
DEPTH REQUIREMENTS:
Major planet sections (Sun, Moon, Ascendant/Lagna) require a complete psychological portrait, not a catalogue. Adequate depth means covering, in full: sign in this specific house; dignity and how it modulates expression; every major aspect with the aspecting planet named and its specific psychological dynamic shown; how this person registers and handles being perceived through this planet (their experience of it, not the audience's verdict — see PROSE FAILURE MODES #3); what the person believes about themselves that may not be accurate. Treat the stated word range as a target, not a quota to fill: a section that pads to reach it fails exactly as badly as one that skips required material. The measure is information delivered per sentence, not length — when the chart's material is genuinely covered, stop, and never inflate to hit a count. A major section runs short only when it has omitted required material, not when it has declined to repeat itself. None of these planets may be shortened on the assumption it is covered elsewhere; each is interpreted in full here.
...
Target 550–750 words; cover every element above, but treat the range as a target, not a quota — do not pad to reach it (see DEPTH REQUIREMENTS). Throughout, anchor each emotional pattern to the situation that activates it — use the SITUATIONAL FRAME in the context to show the moment it surfaces, not only the structural difficulty. Close on something the section has earned but not yet stated — an emotional gift they overlook in themselves, or something they misread about their own emotional nature — named through the kind of situation in which it actually shows up. Do NOT restate the Moon's central note as if newly discovered, do NOT open with a "the truest observation is…" formula, and do NOT end on a struck aphoristic fragment.`,
